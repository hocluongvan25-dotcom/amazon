...
| G2 | Bảng thu thập Rainforest (SERP 20–50 ASIN, CR3/CR5/HHI gộp variation theo brand, Amazon 1P, offers, sales estimation, reviews) + worker collection + collection_runs | ✅ **ĐÃ XONG PHẦN MÁY (15/09)** — migration `0026` (competitor_snapshots/reviews_raw/credit_ledger, RPC enqueue cho analyst + 4 RPC worker service_role, 4 view; review loại PII cả ở parser lẫn RPC; harness BƯỚC 25 PASS), parser thuần + MockIntelligenceProvider + RainforestClient (REST, retry, Collections/webhook), job serp/products/reviews + CLI `research:collect` + cron `/api/cron/research-collect` + webhook, panel tiến độ trên `/research/[id]`; web 274 test + worker 454 test PASS. ⏳ CHỜ VEXIM: thử API thật bằng 100 credit trial (sandbox bị chặn egress) |
| G3 | Engine CR3/CR5/HHI gộp variation theo brand, Amazon 1P, sponsored, review velocity; Tab 2 web (chưa PDF) | ✅ **ĐÃ XONG PHẦN MÁY (15/09)** — `concentration.ts` (gộp parent ASIN, ngưỡng ≥10 organic + ≥70% sales coverage, veto đỏ CR3>65%/1P top3, velocity từ 2 mốc); migration `0027` (3 RPC worker service_role ghi điểm trụ/veto idempotent + unique veto, bộ đếm tự cập nhật; harness BƯỚC 26 PASS); worker tự chấm sau products/webhook; panel Tab 2 `/research/[id]` (KPI, biểu đồ thị phần, bảng brand); web 284 + worker 457 pass. Pain clusters/LLM để G4; Keepa để G7. ⏳ CHỜ VEXIM: lượt quét thật thứ 2 (cách mốc đầu ≥7 ngày) để có velocity |
